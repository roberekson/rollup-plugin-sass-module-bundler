...
function compilerOptionsFromTsConfig(typescript) {
    ...
    const tsconfig = filePath && typescript.readConfigFile(filePath, path => fs.readFileSync(path, 'utf8'));
...
function typescript(_a) {
    ...
    const filter = rollupPluginutils.createFilter(options.include || ['*.ts+(|x)', '**/*.ts+(|x)'], options.exclude || ['*.d.ts', '**/*.d.ts']);
